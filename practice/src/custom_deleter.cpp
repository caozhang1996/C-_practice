 ...
 * @brief 虚函数表指针测试
 *
 * ● 如果使用函数指针作为删除器，删除器的函数签名必须符合 void(*)(T*)
 * 的格式，其中 T 是指向的对象类型。也就是说，删除器函数必须接受一个指向对象类型
 * T 的指针作为参数，并且返回类型为 void
 *
 * ● std::unique_ptr
 * 的删除器类型是通过模板参数传递的，需要在创建对象时显式指定；而
...
{
  std::cout << "[fucntion pointer deleter] Custom deleter is called. Deleting "
               "pointer...\n";
  delete _ptr;
}
...
inline void testLabdmaDeleter()
{
  auto func = [](int* _ptr) {
    std::cout
        << "[lambda deleter] Custom deleter is called. Deleting pointer...\n";
    delete _ptr;
  };
...
struct Deleter
{
  ...
  {
    std::cout
        << "[functor deleter] Custom deleter is called. Deleting pointer...\n";
    delete _ptr;
  }
};
